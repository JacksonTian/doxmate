...
exports.process = function (input, output) {
  ...
  fs.writeFileSync(path.join(output, 'doc/api.html'), ejs.render(api, obj), 'utf8');

  // generate homepage
  var homepage = getTemplates().homepage;
  fs.writeFileSync(path.join(output, 'doc/index.html'), ejs.render(homepage, obj), 'utf8');

  // generate docs
  var doc = getTemplates().doc;
  for (var key in obj.docs) {
    obj.content = obj.docs[key];
    fs.writeFileSync(path.join(output, 'doc/' + key + '.html'), ejs.render(doc, obj), 'utf8');
  }

  // copy styles
  ncp(path.join(__dirname, '../templates/bootstrap'), path.join(output, 'doc/bootstrap'), function () {});
  ncp(path.join(__dirname, '../templates/base.css'), path.join(output, 'doc/base.css'), function () {});
};
