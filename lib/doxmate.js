...
var getTemplates = function (skin) {
  ...
  var configure = require(path.join(__dirname, '../templates/' + skin + '/doxmate.json'));
  templates = {
    'section': section,
    'api': api,
    'doc': doc,
...
exports.process = function (input, output, skin) {
  ...
  if (fs.existsSync(path.resolve(input, 'doxmate-'+skin+'.json'))) {
    obj.options = require(path.resolve(input, 'doxmate-'+skin+'.json'));
  } else {
...
exports.options = function (skin) {
  // copy theme options json
 ncp(path.join(__dirname, '../templates/' + skin + '/doxmate.json'), path.join('.', 'doxmate-'+skin+'.json'), function () {});
}
