...
exports.process = function (input, output, skin) {
  ...
  if (fs.existsSync(path.resolve(input, 'doxmate.json'))) {
    obj.options = require(path.resolve(input, 'doxmate.json'));
  } else {
...
exports.options = function (skin) {
 ...
 ncp(path.join(__dirname, '../templates/' + skin + '/doxmate.json'), path.join('.', 'doxmate.json'), function () {});
}
